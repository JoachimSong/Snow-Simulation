		...
		SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float3 worldPos = _WorldSpaceCameraPos + linear01Depth * r.dir;
				return fixed4(linear01Depth, linear01Depth, linear01Depth, 1.0);*/

				//accumulate density though volume along ray
				float density = 0;
				for (int j = 0; j < NUM_SAMPLES; j++, start += ds)
				{
					if (tex3D(volumeTexture, start).x == 1) {
						density = 1;
						break;
					}
				}
				...
				float3 col = grab * _Color.xyz * density;
				return float4(col, _Color.w);
			}
			ENDCG
		}
	}
